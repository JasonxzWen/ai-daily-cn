...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Daily production entrypoint | B | REC-324 adds an exact-coverage first-pass public-authoring stage between deterministic draft and formal review; focused authoring/pipeline tests pass 53/53, affected daily-runner tests pass 36/36, and final validation passes 903 total / 901 pass / 0 fail / 2 skipped | No Web behavior changed; browser acceptance is correctly skipped under the supported `1280x900` desktop-only policy | Good | The production wrapper owns one schema-constrained authoring call, the host owns path allowlisting/apply/fallback, authored state survives review into repair/degrade/report_write, and first-pass work is accounted separately from exceptional repair | PR/CI and three consecutive post-merge real runs remain; backfills stay separate | 2026-07-13 |
...
| Automated authoring/repair boundary | B | Codex is read-only and returns JSON-Schema-constrained UTF-8 output. First-pass authoring must cover every declared public-prose path exactly once and is applied only when every edit is accepted; follow-up repair receives only error-covered safe public-editorial tasks, while `translation_fidelity` stays advisory | Good | Three consecutive post-merge runs must prove at least two first-review direct passes, median exceptional-repair tasks of zero, and a daily maximum of two; cost tracking and signal-interrupt finalization remain | 2026-07-13 |
