...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Daily production entrypoint | B | The 2026-07-13 scheduled run exposed mixed-task retry, official-blog producer-date, and empty terminal-evidence defects. This Slice closes all three locally; 47/47 affected runner/collector/contract tests, 50/50 entrypoint/context tests, workflow validation, and full validation with 912 total / 910 pass / 0 fail / 2 skipped pass | No Web behavior changed; browser acceptance is correctly skipped under the supported `1280x900` desktop-only policy | Good | Fixtures cover stage receipts, bounded retries, safe task-path authorization, semantic terminal projection, source/context date and hash lineage, and explicit UTF-8 scheduler reads | Fresh merged-main scheduled closure remains; REC-324 first-pass authoring and missing backfills are separate | 2026-07-13 |
| ADC public frontend | A | Desktop-only guard, 6/6 ADC visual contracts, affected suites, typecheck, E2E, build-clean, privacy, and full validation pass; project-owned width breakpoints, viewport meta, mobile/touch branches, and multi-viewport checks are removed | Home, ops, official blogs, and 2026-07-09 report pass at `1280x900`; 4/4 have no console/page/network/HTTP/overflow failures and screenshots are retained under `.tmp` | Good | Static guard covers runtime, generators, Skills, templates, current generated shells, all 49 reports, and exact project selectors in the Astryx bundle | Generic third-party Astryx compatibility remains an explicit non-support exception; desktop cross-browser/a11y hardening remains P2 | 2026-07-13 |
