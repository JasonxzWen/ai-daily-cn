...
| Candidate artifact and source-level contract | B | Focused RED/GREEN passes 5/5; the affected unit/source/Aify/content suite passes 577/577. Final validation passes 923 total / 921 pass / 0 fail / 2 skipped with Harness, build-clean, 194-file privacy, E2E, JSON, and diff gates. The exact 2026-07-14 historical non-publish artifact validates 336/336 at SHA-256 `2f25b5...fb0e`; the optimized draft has zero source-level errors and only its three expected pre-finalization fields absent | Skipped: no Web composition changed, and `1280x900` remains the only supported viewport | Good | Registry, candidate, and report schemas have an order-identical vocabulary; both snake/camel declarations are constrained; story-audit fields and eight legacy editorial aliases are normalized before draft validates the full pool ahead of evidence caching, authoring, or writes | Stacked PR/CI and one fresh merged-main non-publish run remain; historical replay is explicitly not production verification | 2026-07-14 |
...
| Aify logical-source lane | B | Focused Aify contracts pass 8/8, affected Aify/Source Watch tests pass 21/21, runner Phase5 tests pass 2/2, final aggregate passes 910 total / 908 pass / 0 fail / 2 skipped, build-clean, 194-file privacy and desktop E2E; the real current audit truthfully remains 0/3 complete / 0 public matches / not verified | Skipped: no Web composition changed, and the only supported viewport remains `1280x900` | Good | The production summary carries Aify evidence; both governed entries are separately required, and conflicting hybrid identity is auditable but satisfies neither entry, while publisher and aggregator authority remain intact | PR/CI and three post-merge persisted production days with at least one public match remain required | 2026-07-13 |
| GitHub Trending fact lane | B | REC-332 enrichment gate remains green; REC-314 canonical effectiveness RED/GREEN passes 3/3, the affected source suite passes 14/14, the real 2026-07-09 artifact replays from false 70/30 to canonical 50/20, and final validation passes 907 total / 905 pass / 0 fail / 2 skipped with build-clean, 194-file privacy and desktop E2E | Skipped: no Web composition changed; only `1280x900` is supported | Good | Scope/language, same-scope rank, README SHA cache, canonical repository identity, final-section inclusion and audit/effectiveness parity now have deterministic contracts | PR/CI and a fresh persisted merged-main 50/20-style production proof remain; historical bad facts stay immutable | 2026-07-13 |
...
| Selection fact boundary | B | Draft owns scoring and selection; candidates persist private score/rank plus one terminal disposition; report_write, quality, Phase5, source effectiveness, and index status consume or validate those facts. The complete pool is now schema/reference validated at its producer boundary, and source/candidate/report source-level vocabularies are synchronized by regression. | Good | Production verification waits for a fresh merged-main non-publish run; historical rank and replay artifacts remain immutable evidence only | 2026-07-14 |
| Logical-source evidence boundary | B | Aify's content entry and site watch share one stable governance identity while candidates preserve their original publisher and remain `ai_news_aggregator` / `intermediary_only`. Production Phase5 now records a structured terminal summary and requires both entries per day before joining source effectiveness, candidates, dispositions, dated public URLs, and shared admission/lineage | Good | Production observation is intentionally open until three complete post-merge days and one public match; endpoint replay, site-only evidence, or a failed shared Phase5 day cannot close it | 2026-07-13 |
...
- Change: Added strict optional `source_level` and legacy `sourceLevel` contracts to the source registry, synchronized the source/candidate/report vocabularies, classified the six production levels as third-party, and moved full candidate-pool normalization to the draft producer boundary. The new boundary also declares three existing story-cluster audit fields and maps eight explicit legacy editorial aliases to the canonical vocabulary; unknowns still fail closed.
...
- New gaps: Stacked PR/CI and one fresh merged-main non-publish production run remain.
...
- New gaps: Stacked PR/CI, a real non-publish artifact replay, and consecutive merged-main production observations remain.
...
- New gaps: PR/CI and three consecutive post-merge production days remain.
...
- Change: Deduped GitHub original/derived candidates by canonical repository, moved public inclusion truth to the final `github_trending` plus matching `projects` union, and reused the same counter for source audit.
...
- New gaps: PR/CI and one fresh persisted merged-main production proof remain.
- Closed gaps: Derived trend rows and overlapping project rows no longer inflate candidate/included effectiveness; stale candidate flags cannot manufacture public inclusion.
...
- New gaps: PR/CI gates remain. After merge, three consecutive real non-publish observations must show at least two first-review direct passes, median exceptional-repair tasks of zero, no more than two on any day, and zero plain-language blockers before REC-324 can become `production_verified`.
