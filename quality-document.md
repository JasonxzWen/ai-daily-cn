...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Daily production entrypoint | B | P0 38/38 production-entrypoint tests pass for bounded `needs_ai_repair -> structured contract -> resume`, UTF-8, invalid/out-of-scope contracts and evidence, budget exhaustion, verification/admission convergence, latest duplicate-stage truth, successful fallback cleanup, bounded process-tree termination, and honest Source Watch status | Not applicable | Good | Real nested Codex produced a valid contract; the same real discovery artifacts replay cleanly through current admission/quality code | Scheduled clean-origin/main proof requires this PR to land; interrupt/finally terminalization and a real Source Watch producer/consumer remain open | 2026-07-10 |
| ADC public frontend | B | P0 contract tests, typecheck, all-history build, and accepted design artifact gate pass | Passed locally at 1440x900 and 390x844 for home/ops, earliest and latest reports; official-blog desktop also passed, with zero console/network/HTTP/overflow failures | Good | Focused visual/design tests stable | Durable React E2E, broader accessibility, and cross-browser checks remain P2; production Pages evidence waits for delivery | 2026-07-10 |
| Automation operating state | D | Audit only: 52 run worktrees in 15 days; confirmed missing 2026-06-27, 2026-07-07, 2026-07-10 | Not applicable | Poor outside repo | Stale `running` summaries and no self-check evidence | External memory rotation, worktree GC, interrupt terminalization, self-check target repair, and historical backfill need separately authorized operations | 2026-07-10 |
...
| Daily Codex DAG-lite MVP runner | B | P0/P1 passed: 38 target tests, real nested-Codex contract authoring, real-date current-code replay, DAG/workflow/harness gates, and whitespace checks | Deferred; no UI behavior changed | Good | Fixture and production modes cover safe work dirs, host-owned writes, structured output, timeout/process-tree termination, repair budgets, terminal truth, Source Watch semantics, CLI errors, and publish boundaries | Post-merge scheduled publish/Pages evidence, interrupt finalization, multi-agent fanout, and full 16-node DAG migration remain future slices | 2026-07-10 |
