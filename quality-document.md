...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Daily production entrypoint | B | S-73 now keeps advisory-only and stale editorial tasks from vetoing or expanding exhausted-budget safe degradation; the focused regression, related runner cases, isolated unit suite (540/540), and full repository gate (911 total / 909 pass / 0 fail / 2 skipped) pass | No Web behavior changed; browser acceptance is correctly skipped under the supported `1280x900` desktop-only policy | Good | Retry and terminal degradation share one error-covered public-editorial task set; advisory fidelity tasks remain evidence only | PR/CI and fresh scheduled proof remain; repair convergence and artifact replay are separate stacked slices | 2026-07-14 |
...
| Automated authoring/repair boundary | B | Codex is read-only and returns JSON-Schema-constrained UTF-8 output. First-pass authoring must cover every declared public-prose path exactly once; follow-up repair and terminal degradation grant authority only to error-covered safe public-editorial tasks, while `translation_fidelity` stays advisory | Good | Monotonic repair progress, current-feedback isolation, prevalidation, and three consecutive post-merge runs remain | 2026-07-14 |
