...
| Daily Codex DAG-lite MVP runner | B | P0/P1 passed: target runner tests, real-date fixture run, DAG/workflow/harness gates, whitespace check, and full validate | Deferred; no UI behavior changed | Good | `tests/daily-codex-pipeline.test.js` covers six-stage planning, success, unsafe work-dir rejection, repair-once success, Codex repository-write guard, structured unrepaired failure, CLI success/failure, npm positional argument handling, legacy flag rejection, and production-facing script registration | Real Codex CLI execution in CI, final report assembly, publishing, Pages verification, multi-agent fanout, and full 16-node DAG migration remain future slices | 2026-07-06 |
| Daily Codex DAG manifest contract | B | P0 passed for command artifact-proof slice; full `npm run validate` passed for this slice | Deferred; no UI behavior changed | Good | `tests/daily-codex-dag.test.js` covers manifest/run-summary readiness projection, future node execution spec shape, executor/invocation pairing, cwd/prompt path preflight, Codex prompt existence and file-type rejection, blank argv/args token rejection, command invocation policy, controlled command runtime resolution, Codex CLI runtime resolution, generic node runtime dispatch, real deterministic command execution into a validated node result, live `.tmp` JSON output metadata resolution, missing-output failure, runtime-policy invariants, production manifest spec absence, mismatch rejection, no-execution contract-run semantics, and artifact binding guards; targeted DAG test reported 41/41; full validate exited 0 with 718 tests / 716 pass / 0 fail / 2 skipped | Full executable 16-node runner migration, Codex execution, prompt delivery, and business-schema artifact verification remain future work; no production node may carry `node_execution_spec` yet | 2026-07-06 |
...
| DAG-lite runner layer | B | `daily:codex-pipeline` now points at a six-stage replacement runner that writes bounded `.tmp/daily-codex-mvp/YYYY-MM-DD/` artifacts, validates the final MVP JSON, attempts exactly one repair, writes structured blocked summaries on failure, and rejects legacy publish-mode flags instead of silently treating them as compatible | Good | The MVP runner proves orchestration and validation with deterministic fixtures; it does not yet execute live Codex in CI, assemble public daily reports, or publish | 2026-07-06 |
| DAG contract layer | B | `config/daily-codex-dag.json` references resilience policy instead of duplicating failure rules, validates artifact path ownership, checks input paths against upstream outputs, requires per-node `execution_contract.readiness`, projects deterministic execution levels and readiness into dry-run/contract-run summaries, validates run-summary envelopes with a dedicated schema plus semantic validators, defines schema/semantic validation for executable node result contracts, defines a future `execution_contract.node_execution_spec` shape with semantic cwd/prompt path preflight, invocation input policy, command policy, controlled command runtime resolution, explicit Codex CLI runtime resolution, generic node runtime dispatch, command-only executable node closed loop, live helper-level artifact metadata proof, and runtime policy, and exposes the non-publishing contract-run through a workflow-validated npm script | Good | Contract-run deliberately emits skipped node-scope results only; `node_executable` readiness is reserved and rejected in the production manifest; command execution and artifact proof exist only as helper-level synthetic fixture paths; Codex execution, prompt delivery, full command flag parsing, business-schema artifact verification, retry execution, network allowlists, secret scopes, fanout item expansion, barrier aggregation, package/workflow production migration, and npm argument ergonomics remain future work | 2026-07-06 |
...
- Improved: Future command specs now validate that `argv[0]` is the reserved `node` runner, reject shell-ish argv tokens (`&&`, `||`, `;`, `|`, `&`, backticks, redirection, CR/LF, and `$(`), require `argv[1]` to be a safe repo-relative script under `scripts/`, require `.js`/`.mjs`, and verify the script exists as a file. Positive synthetic specs use an existing repo script and still fail only on the reserved `node_executable` gate; full `npm run validate` passed with 703 tests, 701 passed, 0 failed, and 2 skipped.
- Regressed: None known.
