...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Daily production entrypoint | B | P0/P1 focused green: 40/40 pipeline tests, 116/116 affected cross-suite tests, and final full validation 900 / 898 pass / 0 fail / 2 skipped; live `ai-2` workflow compatibility passes with all non-prompt fields preserved | Public browser surfaces accepted; real non-publish production run waits for merged `origin/main` | Good | Fixtures cover stage receipts, fingerprints, date-bound proof, source date, binding integrity, score precedence, stale scheduler assertions, and extra automation rejection | PR/CI, post-merge real run, and three previews remain | 2026-07-10 |
...
| Public article index artifact | B | 12/12 article-index tests plus full build-clean/privacy validation pass for Source Watch projection, dedupe, newest event, denylist, determinism, and fixed-date receipt | Source Watch rail/backrefs and desktop/mobile overflow accepted on a real generated fixture | Good | Stable focused and E2E tests | Post-merge real same-run receipt remains; public all-history and private per-run proof have distinct scopes | 2026-07-10 |
...
- Regressed: no known repository-local P0/P1. Browser acceptance exposed and then closed a black-on-black Source Watch heading; the only retained browser finding is a non-blocking missing favicon request.
- New gaps: PR/CI/merge, post-merge real dry-run/backfill previews, retained storage classification, stale unmanaged DB cache reconciliation, favicon polish, and automatic retention remain.
