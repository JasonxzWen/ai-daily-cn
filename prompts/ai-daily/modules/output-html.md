...
- 内容只按规范 `1280x900` 桌面视口设计和验收。
- 页面包含日期、覆盖时间范围、摘要、主体信息、热门博客、GitHub Trending Top20、X/Twitter 讨论、社区线索、短缺口说明和来源链接；国内/中文动态并入现有事实分组、热门博客、GitHub Trending 或社区线索，不生成独立“国内动态”栏目。
- 公共 HTML 必须保留读者可见的优势：公开 JSON、证据图片、GitHub Trending Top20、Builder 观点卡片和桌面导航；候选回指、source_audit 附录、自检、质量门详情和执行诊断只保留在内部报告、质量报告或交接材料中。
- `summary` 和 Header 必须是读者可读的编辑导语，不得出现“最新 main”“重新生成”“结构化 JSON”“内容单元”等生产过程文案；这些信息只放在内部 `self_check` 或审计字段中。
...
- `hero_highlights` 只是可选亮点数据，不再是“今日必看”或首屏必需结构；不要为了凑亮点从 `summary` 机械截句，也不要写“其余条目见后文”。
- 主体信息目标为 story-first 主列表，默认 8 条、最多 12 条，允许少于 8；每个主题分组默认展开为一个大 cell，内部用标题、2-3 条事实 bullet、证据等级和来源链接展示，不给每条新闻生成默认折叠详情。实体、变化、限制或关键结论可用 `**加粗**` 和 `==高亮词==` 标出。公开页会把高亮词渲染成加粗变色文字，不得把正文关键词做成 tag/chip；tag/chip 只用于重要级别、趋势和星标变化。不要展示“发生了什么：/ 为什么值得看：”、入选条件、候选分数、`reader_relevance`、watch-next、序号详情格或通用来源桶标签。
...
- `projects` 不渲染独立“今日值得关注的项目”板块，也不渲染“项目 highlight / 项目 highlights”标签、子标题或额外项目列表；经过额外核验且匹配 GitHub Trending Top20 的项目，只在对应 Trending 条目里补充领域、作用和 star/release 等行内说明。
...
仓库发布器会先把结构化 `report.json` 转成 `.codex/skills/effective-interact` 的 interaction input，再用 `pre-rendered` 模式生成公开 HTML；公开 JSON 与 HTML 必须同时产出，供验证和 feed 使用。
