...
- 主体信息目标为 8-12 条，默认 10 条；每条只展示标题、2-3 句/行可追溯事实概括和来源链接，实体、变化、限制或关键结论用 `**加粗**` 和 `==高亮词==` 标出。公开页会把高亮词渲染成加粗变色文字，不得把正文关键词做成 tag/chip；tag/chip 只用于重要级别、趋势和星标变化。不要展示入选条件、候选分数、`why_it_matters`、`reader_relevance`、watch-next、序号详情格或通用来源桶标签。
