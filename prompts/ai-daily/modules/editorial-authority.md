...
- `main_items`：固定为 8-12 条短新闻流，默认 10 条；按公共 AI 重要性、科技新闻声量、大厂名气和泛 AI 热点排序，不按用户个人工作直接相关性排序。每条只展示标题、2-3 句/行可追溯事实概括和来源链接。
...
7. `main_items` 固定为 8-12 条短新闻流，公开页不再展示入选理由、`why_it_matters`、`reader_relevance` 或 watch-next 模板字段。
...
11. 主体信息改为 8-12 条短新闻流，每条只保留标题、事实概括和来源链接。
