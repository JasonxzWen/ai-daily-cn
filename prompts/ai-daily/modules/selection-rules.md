...
> 范围声明：本模块仅适用于公共信号流完成后的可选遗留编辑报告（legacy report）。它不治理 `docs/signals/**`，不得改变公共信号流的成员集合、默认时序或发布结果；选择、去重、配额和原帖要求只决定遗留报告如何编辑，不得删除、阻断或延迟安全的公共信号。

- 主体信息目标为 story-first 主列表，默认 8 条、最多 12 条，允许少于 8。每条公开正文展示具体标题、读者可读的事实 bullet、证据等级和来源链接，并用 `**加粗**` 和 `==高亮词==` 标出实体、变化、限制或关键结论。结构化字段可以保留 `what_happened` / `why_it_matters`，但公开页不得显示“发生了什么：/ 为什么值得看：”这类标签。`==高亮词==` 是正文文字强调，不是 tag/chip。不要为了凑数加入低质量内容。
...
- `stories` 按公共 AI 重要性、科技新闻声量、大厂名气和泛 AI 热点排序，不按用户个人工作直接相关性过滤。`why_it_matters` 是兼容 story 字段，必须具体；公开渲染时只作为自然正文或 bullet 的素材，不显示字段标签。`reader_relevance`、入选理由、候选分数和 watch-next 只可作为内部兼容元数据，公开页不得展示。
...
- 遗留报告的 X 热点只有保留原始 X status URL 时才进入候选；没有原始帖链接的热度摘要不进入 legacy 正文，但安全记录仍留在公共信号流，并标记原始链接、可信度与可追溯性缺口。
