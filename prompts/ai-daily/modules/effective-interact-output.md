...
- interaction input 必须保留日报的日期、覆盖时间范围、摘要、主体信息、非空的“技不止术”（字段仍为 `hot_blogs`）、GitHub Trending Top 10、Builder 观察、社区线索、`source_audit`、自检和结构化 JSON 链接；空数组对应板块不要渲染到正文和导航中；国内/中文动态并入现有栏目，不生成独立“国内动态”导航项。
- 正文证据图和“技不止术”卡片图片必须通过 effective-interact 的 lightbox 交互支持点开放大；内嵌来源 icon 不参与放大。
