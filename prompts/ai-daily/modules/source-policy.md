...
- `main_items` 目标是 8-12 条，默认 10 条；每条公开正文只展示标题、2-3 句/行可追溯事实概括和来源链接，并使用 `**加粗**` / `==高亮词==` 标重点。高亮词会渲染成加粗变色文字，不是 tag/chip。主体信息只写新闻事实、数据、图表、限制和影响，不写对日报自身的“为什么重要/启示/后续跟进/报道口径/扩容建议”。
