...
- For reports dated `2026-06-11` or later, missing `source_audit.china_ai_sources.checked:true` blocks strict publish. A checked China AI lane with no qualified recent signal is degraded, not blocking, and must be visible in public source coverage.
...
- Public source coverage must use colored status tags and collapsed per-source details for GitHub Trending, Hugging Face Trending, China AI, blogs, Builder/X, platform lanes, search, and health checks. Do not expose candidate pools, scores, local paths, tokens, or internal debug logs.
