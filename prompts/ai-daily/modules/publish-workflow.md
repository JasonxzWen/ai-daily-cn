...
- 当 `next_action.kind` 是 `codex_ai_repair_contract` 时，Codex 作为 AI 执行者写入 runner 指定的 contract 路径；contract 必须包含 `schema_version`、`report_date`、`status:"ready"` 和非空 `edits`，然后用同一命令继续 runner。若 runner 已创建 `status:"template"` attempt 文件，只在该新文件里补齐必要 public text edits 并改为 `ready`，不要覆盖上一轮 contract；runner 会从 summary 恢复，应用 ready contract，复查质量，并把 `.tmp/daily-report.optimized.json` 作为后续 `report:write` 输入。
