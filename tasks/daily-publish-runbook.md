...
- Production Source Watch has no connected producer/consumer handoff. Scheduled publish must omit the fixture-only artifact flag and treat `source_watch.production_status:"not_connected"` plus `consumed:false` as authoritative. `source_watch_requested_artifact_path` is diagnostic input only; `source_watch_admitted_artifact_path` stays empty until production consumption is proven.
