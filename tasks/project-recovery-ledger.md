...
- Current-Slice action: the workflow validator requires evidence-backed Source Watch fields, rejects permanent old scheduler assertions and extra project automations, and treats `status:self-check` as manual-only. Focused/full gates cover producer-stage path/hash receipt, producer/pool fingerprints, fixed-date proof, material endpoints, latest event, official-blog source date/context/bindings hashes and relationships, score precedence, public/private fields, E2E schema freshness, Source Watch contrast, and canonical `1280x900` desktop layout. The source registry, candidate pool, and report now share an order-identical `source_level` vocabulary under a synchronization regression; both snake/camel declarations are constrained, and draft validates the full candidate pool before any authoring or artifact write. A provenance-bound 2026-07-14 replay fixture remains `production_verified:false` while the exact external 336-candidate artifact passes read-only validation at its original SHA-256. Final validation passes 923/921/0/2.
- Blocker: source-level recurrence prevention is locally verified; stacked PR/CI, a fresh merged-main non-publish run, broader latest-report artifact coverage, and longer-term alerts/retention monitoring remain.
...
- Acceptance: high-risk WeChat/SSPAI fixtures block; disclosed low-risk signals remain attributable; verified GitHub/bioRxiv targets upgrade; Phase5 reports only final-report truth; registry validation cannot admit a source level that candidate/report schemas reject; candidate producer validation precedes authoring and writes; quality pass implies report_write cannot later fail candidate coverage.
...
- Recurrence prevention: the feature inventory, production stage and Phase5 audit fail closed on missing days, missing logical rows, either missing required entry/candidate, conflicting source/watch entry identities, missing dispositions, or included/public mismatches; endpoint reachability alone never counts as production proof.
...
| S-83 | the 2026-07-14 non-publish run produced 32 candidates with four configured `source_level` values rejected by candidate/report schemas; registry validation ignored snake_case and let camelCase unknowns bypass the first fix, while three invalid candidates had already entered `community_leads`. Moving validation earlier also exposed three runtime story-audit fields missing from schema and eight legacy editorial aliases | added a strict synchronized registry/candidate/report vocabulary, constrained snake/camel declarations, preserved fallback metadata, classified six configured levels as third-party, declared the runtime audit fields, normalized only known editorial aliases, validated the full pool at the draft producer boundary, and retained a provenance-bound historical replay receipt | locally_verified under REC-401/REC-311; focused 5/5, affected 577/577, exact 336-candidate SHA-bound replay, final 923/921/0/2 and independent P0/P1=0 all pass; stacked PR/CI/fresh merged-main run pending |
