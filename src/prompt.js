...
export async function assemblePrompt(options = {}) {
  ...
  parts.push("- 最终公开主产物是 `docs/signals/index.json`、分组分页 JSON 与 React 公共信号页。");
  parts.push("- 结构化日报数据继续写入 `reports-data/YYYY/MM/YYYY-MM-DD.json`，公开信息副本写入 `docs/data/YYYY/MM/YYYY-MM-DD.json`；不再生成独立日报 HTML。");
  ...
  parts.push("- 真实发布后必须验证 GitHub Pages 首页和当日公开 JSON 返回 HTTP 200。");
