...
function normalizeAutodraftPublicText(report) {
  ...
  report.self_check.notes = "已完成候选池写回；高风险或中介线索不进入主体事实。";
...
function summaryForSelection(selection, aigcCount) {
  ...
  if (aigcCount > 0) {
    parts.push(`其中 ${aigcCount} 条直接关系内容生成或创作者工具`);
  }
  if (selection.github_trending.length > 0) {
    parts.push(`另外保留 ${selection.github_trending.length} 个 GitHub Trending Top 10 项目，用来补开源侧信号。`);
  }
  if (selection.hot_blogs.length > 0) {
    parts.push(`技不止术补了 ${selection.hot_blogs.length} 条深读，用来承接产品拆解、平台策略和工作流观察。`);
  }
...
function mainItem(candidate, original) {
  ...
  return {
    ...
    bullets: [
      ...
      detail ? `==落点==：${ensureChineseSentence(detail)}` : null
    ].filter(Boolean),
...
function communityLeadPublicSummary(candidate) {
  ...
  return `${source} 这条线索指向${theme}，当天公开细节主要还是${scope}，暂时先放在观察区。`;
}
...
function canPromoteToMain(candidate) {
  ...
  const sourceLevel = sourceLevelForCandidate(candidate);
  const allowReaderRelevantCompanySignal = sourceLevel === "official_company_news" && isReaderRelevantCandidate(candidate);
...
function chineseGithubDescription(description, repo) {
  ...
  return trimText(`${repoLabel} 今天进了 GitHub Trending Top 10，仓库描述显示它主打${pitch}，可以当作这类工具最近的一份实现样本来看。`, 120);
}
...
export function hotBlogSummary(candidate) {
  const text = candidateText(candidate).toLowerCase();
  if (/gpt-5\.2 and gpt-5\.2-codex deprecated/.test(text)) {
    return trimText("这条更新的重点不是单纯下线两个模型名，而是 GitHub 开始重新整理 Copilot Chat、补全和 agent 模式背后的默认模型组合。真正值得看的，是哪些入口已经切走、哪些团队还需要补迁移、以及现有提示词、评测和自动化流程会不会被连带影响。", 220);
  }
  if (/enterprise-managed plugins in vs code in public preview/.test(text)) {
    return trimText("VS Code 这次放出的不是一个新插件，而是企业统一管理插件装配和策略的入口，核心问题是开发环境治理终于开始产品化。对平台团队来说，更值得盯的是权限模型、允许名单、分发方式和团队级配置能不能真正落到现有 IDE 管理流程里。", 220);
  }
  if (/unlocking dependable responses with gemini enterprise agent platform.?s agentic rag/.test(text)) {
    return trimText("Google 这篇文章拆解的是企业检索增强的落地流程：先准备知识切片和索引，再处理查询改写、召回、重排与上下文拼装，目标是减少幻觉并稳住回答质量。更有用的是，它把效果验证和方法边界也放进同一套流程里，团队可以据此判断企业知识库问答到底该在哪些环节补检索和评测。", 220);
  }
  if (/productivity agent suite|gateway to ai for users and enterprises/.test(text)) {
    return trimText("腾讯云把 Productivity Agent Suite 打包成企业接入 AI 的统一入口，套件把 agent、模型和业务流程放在同一条交付链路里。文章真正有价值的地方，是它把哪些组件、业务场景和企业接入路径公开出来，说明大厂正把企业级 agent 从概念演示往标准化产品推进。", 220);
  }
  if (/dragonwell native acceleration|java\/scala applications|performance analytics system/.test(text)) {
    return trimText("阿里云介绍一套 AI 驱动的性能分析系统，用来自动找出 Java / Scala 应用里最值得优先优化的热点。重点不是“10 倍提升”的口号，而是把性能排查、定位和优化建议做成可重复的产品能力；如果方法和误报边界公开得足够清楚，这类工具就可能进入日常性能治理链路。", 220);
  }
  if (/openclaw|telegram|jalankan ai coding agent|coding agent anda sendiri/.test(text)) {
    return trimText("这篇教程把 OpenClaw、阿里云 ECS 和 Telegram 串成一条可直接上手的自建 coding agent 路线，说明个人或小团队已经能用现成云资源快速搭起远程协作入口。真正有用的是部署步骤、权限配置和协作体验是否写得足够清楚，而不是再讲一遍“agent 很火”。", 220);
  }
  if (/nemotron 3\.5 content safety|customizable multimodal safety/.test(text)) {
    return trimText("这篇文章介绍 Nemotron 3.5 Content Safety 如何把文本、图像等多模态内容做成统一安全分类，并按企业政策返回审核结果。对内容平台和企业 AI 团队来说，更关键的是它把地区、行业和平台差异做成可配置策略，审核口径不必每次重建一套流程。真正需要盯的是误判率、漏判成本、支持的模态范围，以及接入现有风控系统时要补多少人工兜底。", 220);
  }
  if (/hf cli|agent-optimized way to work with the hub/.test(text)) {
    return trimText("这篇文章想解决的，是 agent 操作 Hugging Face Hub 时总被网页按钮、权限跳转和人工确认打断，所以把拉模型、下数据集、登录和仓库管理尽量收进命令行。对做自动化的团队来说，关键不只是多一个 CLI，而是常用 Hub 动作终于更容易被脚本和工作流稳定调用。", 220);
  }
  if (/endava.*software delivery|software delivery.*ai agents/.test(text)) {
    return trimText("OpenAI 用 Endava 案例讲企业怎么把 ChatGPT Enterprise、Codex 和 agent 工作流塞进软件交付链路，重点不是口号，而是哪些开发、协作和自动化环节真的被改写。文章真正有参考价值的地方，是它把落地团队、接入工具和可复用流程说得有多具体，这决定 agent 进入交付体系到底是不是可复制能力。", 220);
  }
...
function builderReadableSummary(originalText) {
  ...
  return "原帖在讨论一个具体的 AI 产品、平台或 agent 工作流变化，但当前自动摘要还不足以公开发布；需要回到原文确认功能边界、对象和限制后再决定是否保留。";
}
