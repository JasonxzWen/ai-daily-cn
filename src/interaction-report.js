...
function formatMainItems(items, context = {}) {
  ...
  return items
    .map((item, index) => {
      const bullets = item.bullets.map((bullet) => `  - ${formatDailyInlineText(bullet, item)}`).join("\n");
...
function formatHotBlogCards(items, context = {}) {
  return items.map((item) => {
    ...
    return {
      ...
      points: points.map((value, index) => ({ label: `要点 ${index + 2}`, value })),
...
function formatTwitterDiscussion(items, auditGroup, options = {}) {
  if (items.length > 0) {
    const content = items
      .map((item) => `- **${item.author}**${formatHighlightTags([importanceTagFor("builder_observations", item)].filter(Boolean))}${item.role ? `（${item.role}）` : ""}：${formatDailyInlineText(item.content, item)} ${markdownLink(item.url, item.source || "X/Twitter")}`)
      .join("\n");
...
function formatCommunityLeads(items, options = {}) {
  ...
  const content = leads.map((item) => `- ${formatHighlightTags([importanceTagFor("community_leads", item)].filter(Boolean))}${formatDailyInlineText(item.content, item)} ${markdownLink(item.url, "来源")}`).join("\n");
