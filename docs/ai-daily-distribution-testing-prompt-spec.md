...
- URL 必须是绝对 URL。
- `main_items` 最少 5 条、目标 8 条、最多 12 条；不能为了凑数引入低质量内容。
...
| 规则 | 失败处理 |
|---|---|
| `main_items` 最少 5 条、目标 8 条、最多 12 条 | 少于 5 条时允许 fallback，并写生成期短缺事件；超过 12 条时合并、去重或降级 |
...
- 验证唯一支持的 `1280x900` 桌面视口没有明显横向破版，且主内容宽度可读。
- 验证如果有筛选或展开控件，交互有效。
