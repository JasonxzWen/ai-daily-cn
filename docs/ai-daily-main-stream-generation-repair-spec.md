...
> **2026-07-14 适用范围变更：仅保留为可选遗留编辑报告的主体生成规格。** 本文中的最少 5 条、目标 8 条、最多 12 条、黑名单、补位、排序、合并、去重、拒绝原因和质量事件都不得控制 PR6 前当前 `docs/signals/**` 或新 pool/edition 的成员集合、默认时序和发布。PR6 前现状仍由 `public-signal-stream-contract:v1` 描述；2026-07-14 方案曾把公开渲染面删除排在旧 PR3，该时序已被 [精选首页三层迁移规格](ai-daily-curated-homepage-migration-spec.md) 取代：当前渲染面保留到 PR6 原子切换，PR3 只运行 source/raw/funnel shadow。
>
...
- 默认窗口：当天。
- 主体不足 5 条时：扩展到 72 小时。
- 仅在遗留编辑报告内部，热门博客、GitHub 项目、研究/工程深读可按源级 `lookback_days` 放宽，但不能重复包装旧内容；公共 occurrence 不读取该窗口。
