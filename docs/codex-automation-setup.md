...
- `direct Chinese RSS`：默认只保留直接可用的中文 RSS/Feed 线索，例如 QbitAI、InfoQ CN、36Kr，以及经 `sources:health` 证明为 RSS/XML 的 Machine Heart 等源。不要恢复 RSSHub、WeChat aggregator、Zhihu 抓取或旧 `config/sources/wechat-whitelist.json` 模板。
