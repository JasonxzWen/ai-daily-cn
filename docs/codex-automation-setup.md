...
13b. 从 `2026-06-02` 起，发布链路会执行严格覆盖门：最终 JSON 必须带有效 `self_check.automation_revision`，`source_audit.content_sources` 至少证明固定 A-F 信源面已检查，GitHub Trending 必须有 Top 10 与 daily/weekly/language 审计来源，Builder 必须包含 follow-builders X 审计和至少 1 条原始 `x.com`/`twitter.com` status，`evidence_assets` 必须至少有 1 张匹配正文条目的本地图片，模型发布必须同步进入 `main_items`。缺这些内容时 `publish:dry-run`、真实 `publish` 和 `publish:github-api` 都会失败；不要把它当成发布环境问题或用兜底绕过。
13c. 固定信源面的目标是证明“已检查并写入审计”，不是要求每个公开源在当天都成功返回内容。对 Reddit、公开网页、RSS 等可能按环境返回 403/5xx 的来源，必须把 `status:"blocked"`、HTTP 状态和 notes 写入 `source_audit`；这可作为 source-surface proof，但不得把 blocked 来源的未核验内容强行写入事实栏目。
13d. 如果在生成日报过程中修改了发布器、质量门、渲染器、提示词或信源配置，必须在代码提交并 push 后重新运行 `npm run report:write -- .tmp/daily-report.json reports-data YYYY-MM-DD` 和 `npm run build`，确保最终 JSON 的 `self_check.automation_revision.git_commit` 等于当前 `HEAD`。不要发布记录旧 commit 的日报。
...
14a. `publish:dry-run` 还必须证明 `current_dirty_files` 中所有发布器管理文件都出现在 `will_stage_files`；如果出现 `publisher_dirty_outside_publish_plan`，先修复发布计划或归档与本次日期无关的悬空产物，再继续。尤其要确认 `docs/trends.json`、`docs/feed.json`、`docs/index.html`、当日 `docs/data/**`、`docs/reports/**`、`reports-data/**` 和日报引用的 `docs/assets/evidence/**` 图片都进入本次 stage 计划。
