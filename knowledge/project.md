...
- 日更 runner 在旧公开信号持久化前运行一个不改变公开页面的 Phase 1A 影子阶段：按日以确定性 gzip JSON 保存 repo-safe 原始观察，并保存 `registered → fetched → parsed` 信源漏斗；正常影子失败及有确定性 recovery evidence 的原子事务失败只记录降级，不阻断旧发布器。无恢复证据的 receipt 损坏、canonical reconciliation、lineage 或 privacy 漂移继续按仓库完整性失败阻断。
- Phase 1B 在 Phase 1A 后运行不阻断旧发布器的确定性准入影子阶段：逐项记录 `admitted / rejected / needs_review` 回执，将跨日去重后的信号以确定性 gzip JSON 保存到内部 signal pool，并生成同代、可校验但不进入当前站点发现与公开信号 schema 的 public-ready 伴随投影。
...
- 日报 Pages 发布与默认可见性验证统一使用 `data/YYYY/MM/YYYY-MM-DD.json`；已废弃的 `reports/YYYY/MM/YYYY-MM-DD.html` 不再生成，也不能作为 `publish:verify-pages --date` 的默认验证目标，显式 `--url` 仍可覆盖。
- 首轮 authoring contract 部分应用时，runner 会保留实际写出的安全编辑并把 authored report 交给正式质量审查，只有零安全编辑或缺少部分结果时才回退原始草稿；不同 Builder 原帖复用同一翻译会在质量审查中阻断，并把后续重复项路由到精确翻译修复。
- legacy 日报的 AI repair contract 部分应用时，只要至少一个编辑已安全应用、剩余问题仍有公开文案修复任务且进度严格减少，runner 会保留成功编辑并生成下一轮 contract；后续 review 停滞且分类明确禁止回滚时也继续保留已接受编辑。自动修复提示直接复用确定性公开文案禁词表。外层 summary 也会从明确的嵌套 legacy handoff 恢复该状态，全部编辑被拒或没有进度时仍按既有安全门阻断或降级。
...
- 热文先执行重复标题和模板化文案剪枝；只要后续仍有合格候选，就继续回填到目标数量，而不是把剪枝损失误报为来源覆盖不足。
- 公开信号的 canonical 历史按产品契约保留在 baseline 与每日 occurrence store；raw observations、source funnel、内部 signal pool 及其 public-ready 伴随投影属于可再生的影子回执，不作为已发布历史长期保留。每日 raw observations、occurrence store 与内部 signal pool 使用无损确定性 gzip，公开 JSON 路径保持不变。总 tracked payload 的硬上限为 384 MiB，reports-data、单文件、重复资产和 Git pack 仍受各自独立门槛约束，不能通过删除 canonical 已发布信号历史绕过体积治理。
...
- [压缩 JSON 编解码](../src/compressed-json.js)
