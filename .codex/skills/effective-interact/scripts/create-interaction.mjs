...
function renderCardTitle(item) {
  ...
  const icon = safeDataImage(item.titleIcon || item.title_icon || item.icon || "");
  const iconHtml = icon ? `<img class="inline-site-icon" src="${escapeAttr(icon)}" alt="" decoding="async">` : "";
  if (!href) {
